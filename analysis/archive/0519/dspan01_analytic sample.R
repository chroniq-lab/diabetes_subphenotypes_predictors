...
jhs_longitudinal <- readRDS(paste0(path_diabetes_subphenotypes_predictors_folder,"/working/cleaned/dsppre01e_jhs.RDS")) %>% 
  mutate(study = "jhs",
         race_clean = "NH Black")
# cimbine dpp + dppos?
dppos_longitudinal <- bind_rows(
  readRDS(paste0(path_diabetes_subphenotypes_predictors_folder,"/working/cleaned/dsppre01c_dpp.RDS")),
  readRDS(paste0(path_diabetes_subphenotypes_predictors_folder,"/working/cleaned/dsppre01d_dppos.RDS"))
) %>% 
  mutate(study = "dppos",
         race_clean = race_eth)
